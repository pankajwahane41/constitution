...
const AmendmentTimelineGame: React.FC = () => {
  ...
  const useHint = (amendment: Amendment) => {
    setGameState(prev => {
  ...
  const renderIntro = () => (
    <div className="max-w-4xl mx-auto p-6">
      <div className="bg-gradient-to-br from-blue-50 to-indigo-100 rounded-2xl p-8 text-center">
        ...
        <button
          onClick={startGame}
          className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold py-4 px-8 rounded-xl text-lg transition-all duration-200 transform hover:scale-105 flex items-center gap-3 mx-auto"
        >
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <h2 className="text-2xl font-bold mb-4">Amendment Cards</h2>
          <div className="space-y-3">
            {gameState.availableAmendments.map((amendment) => (
              <div
                key={amendment.id}
                onClick={() => handleAmendmentClick(amendment)}
                className={`${CATEGORY_COLORS[amendment.category]} bg-opacity-10 border-2 border-opacity-30 p-4 rounded-xl cursor-pointer hover:shadow-lg transition-all duration-200 transform hover:scale-105 min-h-[64px] ${
                  gameState.selectedAmendment?.id === amendment.id ? 'ring-4 ring-yellow-400 ring-offset-2 scale-105 border-yellow-500' : ''
                }`}
                style={{ touchAction: 'manipulation' }}
              >
                <div className="flex justify-between items-start mb-2">
                  <span className={`px-2 py-1 rounded text-xs font-bold ${CATEGORY_COLORS[amendment.category]} text-white`}>
                    {amendment.category.toUpperCase()}
                  </span>
                  <span className={`text-xs font-bold ${DIFFICULTY_COLORS[amendment.difficulty]}`}>
                    {amendment.difficulty.toUpperCase()}
                  </span>
                </div>
                <div className="flex items-start gap-3">
                  <div className="flex-1">
                    <h3 className="font-bold text-lg mb-2">{amendment.title}</h3>
            ...
            <div className="relative">
              ...
              <div className="space-y-8">
                {[1951, 1976, 1978, 1988, 1992, 2002, 2016].map((year, index) => (
                  ...
                  >
                    <div className="absolute left-6 w-5 h-5 bg-white border-4 border-blue-500 rounded-full z-10"></div>
                    <div className="ml-16 flex-1">
                      <div
                        ref={el => dropZoneRefs.current[index] = el}
                        onClick={() => handleYearClick(year)}
                        className={`min-h-[100px] bg-gray-50 rounded-lg border-2 p-4 transition-all duration-200 cursor-pointer ${
                          gameState.selectedAmendment 
      ...
      {gameState.selectedAmendment && (
        <div className="mt-6 bg-white rounded-xl p-6 shadow-lg">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="text-2xl font-bold">{gameState.selectedAmendment.title}</h3>
              <p className="text-gray-600">{gameState.selectedAmendment.year} • {gameState.selectedAmendment.category}</p>
            </div>
            <button
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <h4 className="font-bold text-lg mb-2">What it means:</h4>
              <p className="text-gray-700">{gameState.selectedAmendment.description}</p>
            </div>
    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="bg-gradient-to-br from-green-50 to-emerald-100 rounded-2xl p-8 text-center">
        ...
        <div className="flex gap-4 justify-center">
          <button
            onClick={startGame}
            className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold py-3 px-6 rounded-xl transition-all duration-200 transform hover:scale-105"
          >
            Play Again
          </button>
          <button
