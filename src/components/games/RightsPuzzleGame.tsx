...
const RightsPuzzleGame: React.FC<RightsPuzzleGameProps> = ({ 
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-gradient-to-br from-pink-100 via-purple-50 to-blue-100 min-h-screen">
      {/* Header with Back Button */}
      <div className="flex items-center justify-between mb-8">
        {onBack && (
          <button
            onClick={onBack}
            className="p-3 rounded-lg text-gray-600 hover:text-gray-900 hover:bg-white hover:shadow-md transition-all duration-200"
          >
            ← Back to Games
          </button>
        )}
      ...
      {gameStarted && (
        ...
        >
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div className="bg-gradient-to-br from-red-100 to-pink-100 rounded-lg p-3 border border-red-200">
              <div className="text-2xl font-bold text-red-600">{score}</div>
              <div className="text-sm text-red-800">🎯 Score</div>
            </div>
            <div className="bg-gradient-to-br from-blue-100 to-cyan-100 rounded-lg p-3 border border-blue-200">
              <div className="text-2xl font-bold text-blue-600">{formatTime(timeElapsed)}</div>
              <div className="text-sm text-blue-800">⏱️ Time</div>
            </div>
            <div className="bg-gradient-to-br from-purple-100 to-indigo-100 rounded-lg p-3 border border-purple-200">
      ...
      {!gameStarted ? (
        ...
        >
          <div className="bg-white rounded-lg shadow-lg p-8 mb-6">
            ...
            <button
              onClick={startGame}
              className="bg-gradient-to-r from-red-400 via-yellow-400 via-green-400 via-blue-400 to-purple-500 hover:from-red-500 hover:via-yellow-500 hover:via-green-500 hover:via-blue-500 hover:to-purple-600 text-white font-bold py-4 px-8 rounded-lg text-xl transition-all duration-300 transform hover:scale-105 shadow-lg animate-pulse"
            >
              🎮 Start Game
            </button>
          </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                <AnimatePresence>
                  {availableRights.map((right) => (
                    <motion.div
                      ...
                      className={`${right.color} p-4 rounded-lg shadow-md cursor-pointer text-white transform transition-all duration-200 hover:shadow-lg hover:scale-105 min-h-[64px] ${
                        selectedItem?.id === right.id ? 'ring-4 ring-yellow-400 ring-offset-2 scale-105' : ''
                      }`}
              ...
              {selectedItem && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="mt-4 p-3 bg-yellow-100 border border-yellow-300 rounded-lg text-sm text-yellow-800"
                >
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {categories.map((category) => (
                <motion.div
                  key={category.id}
                  className={`${category.color} rounded-lg shadow-lg p-6 min-h-[200px] transition-all duration-300 cursor-pointer ${
                ...
                >
                  ...
                  <div className="space-y-2 min-h-[100px]">
                      ...
                      .map(([rightId]) => {
                        ...
                        return (
                          <motion.div
                            ...
                            initial={{ opacity: 0, scale: 0.8 }}
                            animate={{ opacity: 1, scale: 1 }}
                            className="bg-white bg-opacity-20 backdrop-blur-sm p-3 rounded-lg text-white"
                          >
                    ...
                    {Object.values(placedItems).filter(cat => cat === category.id).length === 0 && (
                      <div className="border-2 border-dashed border-white border-opacity-50 rounded-lg p-4 flex items-center justify-center min-h-[60px]">
            ...
            >
              <div className="text-center">
                ...
                <div className="flex gap-4">
                  <button
                    onClick={startGame}
                    className="flex-1 bg-gradient-to-r from-red-400 via-yellow-400 via-green-400 via-blue-400 to-purple-500 hover:from-red-500 hover:via-yellow-500 hover:via-green-500 hover:via-blue-500 hover:to-purple-600 text-white font-bold py-3 px-6 rounded-lg transition-all duration-300 transform hover:scale-105"
                  >
                    🔄 Play Again
                  </button>
                  <button
                    onClick={resetGame}
                    className="flex-1 bg-gradient-to-r from-gray-400 to-gray-600 hover:from-gray-500 hover:to-gray-700 text-white font-bold py-3 px-6 rounded-lg transition-all duration-300 transform hover:scale-105"
                  >
                    🏠 Home
                  </button>
                </div>
