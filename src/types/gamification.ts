...
export interface UserProfile {
  ...
  // 10-Day Curriculum Tracking
  curriculumStartDate?: string;     // When user started the 10-day curriculum
  curriculumEnabled: boolean;       // Whether user is following curriculum mode
  curriculumDayCompleted: number;   // Which curriculum day was last completed (0-9)
  curriculumTopicsCompleted: string[]; // Array of completed topic IDs
}
...
export interface DailyChallenge {
  ...
  title: string;
  description: string;
  type: 'quiz' | 'story' | 'mini_game' | 'constitution_builder' | 'curriculum_quiz' | 'curriculum_story' | 'curriculum_mini_game' | 'curriculum_progress' | 'streak';
  target: number;
  progress: number;
  isCompleted: boolean;
  reward: Reward;
  ...
  // Traditional challenge properties
  ...
  isCurriculumChallenge?: boolean;
  curriculumDay?: number;
  categories?: string[];
  keyTopics?: string[];
  storyChapter?: number;
  gameFocus?: string;
  isProgressChallenge?: boolean;
  curriculumProgress?: {
    currentDay: number;
    totalDays: number;
